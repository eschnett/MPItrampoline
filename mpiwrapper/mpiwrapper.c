...
// All is fine
#elif defined OMPI_MAJOR_VERSION && OMPI_MAJOR_VERSION >= 5
// OpenMPI 5 is close enough to MPI 4.0
#else
...
int MPIABI_Comm_detach_buffer(MPIABI_Comm comm, void *buffer_addr, int *size) {
  int ierr = MPI_Comm_attach_buffer(abi2mpi_comm(comm), buffer_addr, size);
  return mpi2abi_errorcode(ierr);
}
...
int MPIABI_Comm_detach_buffer_c(MPIABI_Comm comm, void *buffer_addr,
                                MPIABI_Count *size) {
  MPI_Count mpi_size;
  int ierr = MPI_Comm_attach_buffer(abi2mpi_comm(comm), buffer_addr, &mpi_size);
  *size = mpi_size;
  return mpi2abi_errorcode(ierr);
}
...
int MPIABI_Request_get_status_some(int incount,
  ...
  int ierr = MPI_Request_get_status_some(
      incount, array_of_mpi_requests, outcount,
      ignore_statuses ? MPI_STATUSES_IGNORE : (MPI_Status *)array_of_statuses);
...
int MPIABI_Session_detach_buffer_c(MPIABI_Session session, void *buffer_addr,
                                   MPIABI_Count *size) {
  MPI_Count mpi_size;
  int ierr = MPI_Session_detach_buffer(abi2mpi_session(session), buffer_addr,
                                       &mpi_size);
  *size = mpi_size;
  return mpi2abi_errorcode(ierr);
}
...
int MPIABI_Session_iflush_buffer(MPIABI_Session session,
                                 MPIABI_Request *request) {
  MPI_Request mpi_request;
  int ierr = MPI_Session_flush_buffer(abi2mpi_session(session), &mpi_request);
  *request = mpi2abi_request(mpi_request);
  return mpi2abi_errorcode(ierr);
}
...
int MPIABI_Alltoallv_c(const void *sendbuf, const MPIABI_Count sendcounts[],
  ...
  ierr = MPI_Alltoallv_c(sendbuf, mpi_sendcounts, mpi_sdispls,
                         abi2mpi_datatype(sendtype), recvbuf, mpi_recvcounts,
                         mpi_rdispls, abi2mpi_datatype(recvtype),
                         abi2mpi_comm(comm));
  return mpi2abi_errorcode(ierr);
}
...
int MPIABI_Alltoallw_c(const void *sendbuf, const MPIABI_Count sendcounts[],
  ...
  for (int n = 0; n < size; ++n)
    mpi_recvtypes[n] = abi2mpi_datatype(recvtypes[n]);
  ierr = MPI_Alltoallw_c(sendbuf, mpi_sendcounts, mpi_sdispls, mpi_sendtypes,
...
int MPIABI_Alltoallw_init_c(const void *sendbuf,
  ...
  for (int n = 0; n < size; ++n)
    mpi_recvtypes[n] = abi2mpi_datatype(recvtypes[n]);
  MPI_Request mpi_request;
  ierr = MPI_Alltoallw_init_c(sendbuf, mpi_sendcounts, mpi_sdispls,
...
int MPIABI_Gatherv_c(const void *sendbuf, MPIABI_Count sendcount,
  ...
  for (int n = 0; n < size; ++n)
    mpi_displs[n] = displs[n];
  ierr = MPI_Gatherv_c(sendbuf, sendcount, abi2mpi_datatype(sendtype), recvbuf,
                       mpi_recvcounts, mpi_displs, abi2mpi_datatype(recvtype),
                       root, abi2mpi_comm(comm));
  return mpi2abi_errorcode(ierr);
}
...
int MPIABI_Ialltoallv_c(const void *sendbuf, const MPIABI_Count sendcounts[],
  ...
  for (int n = 0; n < size; ++n)
    mpi_rdispls[n] = rdispls[n];
  MPI_Request mpi_request;
  ierr = MPI_Ialltoallv_c(sendbuf, mpi_sendcounts, mpi_sdispls,
                          abi2mpi_datatype(sendtype), recvbuf, mpi_recvcounts,
                          mpi_rdispls, abi2mpi_datatype(recvtype),
                          abi2mpi_comm(comm), &mpi_request);
  *request = mpi2abi_request(mpi_request);
...
int MPIABI_Ialltoallw_c(const void *sendbuf, const MPIABI_Count sendcounts[],
  ...
  for (int n = 0; n < size; ++n)
    mpi_recvtypes[n] = abi2mpi_datatype(recvtypes[n]);
  MPI_Request mpi_request;
  ierr = MPI_Ialltoallw_c(sendbuf, mpi_sendcounts, mpi_sdispls, mpi_sendtypes,
                          recvbuf, mpi_recvcounts, mpi_rdispls, mpi_recvtypes,
                          abi2mpi_comm(comm), &mpi_request);
  *request = mpi2abi_request(mpi_request);
  return mpi2abi_errorcode(ierr);
...
int MPIABI_Igatherv_c(const void *sendbuf, MPIABI_Count sendcount,
  ...
  for (int n = 0; n < size; ++n)
    mpi_displs[n] = displs[n];
  MPI_Request mpi_request;
  ierr = MPI_Igatherv_c(sendbuf, sendcount, abi2mpi_datatype(sendtype), recvbuf,
                        mpi_recvcounts, mpi_displs, abi2mpi_datatype(recvtype),
...
int MPIABI_Ireduce_scatter_c(const void *sendbuf, void *recvbuf,
  ...
  for (int n = 0; n < size; ++n)
    mpi_recvcounts[n] = recvcounts[n];
  MPI_Request mpi_request;
  ierr = MPI_Ireduce_scatter_c(sendbuf, recvbuf, mpi_recvcounts,
                               abi2mpi_datatype(datatype), abi2mpi_op(op),
                               abi2mpi_comm(comm), &mpi_request);
  *request = mpi2abi_request(mpi_request);
  return mpi2abi_errorcode(ierr);
...
int MPIABI_Iscatterv_c(const void *sendbuf, const MPIABI_Count sendcounts[],
  ...
  for (int n = 0; n < size; ++n)
    mpi_displs[n] = displs[n];
  MPI_Request mpi_request;
  ierr = MPI_Iscatterv_c(sendbuf, mpi_sendcounts, mpi_displs,
...
static MPIABI_User_function *user_fn_0 = NULL;
static void wrap_user_fn_0(void *invec, void *inoutvec, int *len,
                           MPI_Datatype *datatype) {
  MPIABI_Datatype abi_datatype = mpi2abi_datatype(*datatype);
  user_fn_0(invec, inoutvec, len, &abi_datatype);
}
...
int MPIABI_Op_create(MPIABI_User_function *user_fn, int commute,
  ...
  assert(!user_fn_0);
  user_fn_0 = user_fn;
  int ierr = MPI_Op_create(wrap_user_fn_0, commute, &mpi_op);
  *op = mpi2abi_op(mpi_op);
  return mpi2abi_errorcode(ierr);
}

static MPIABI_User_function_c *user_fn_c_0 = NULL;
static void wrap_user_fn_c_0(void *invec, void *inoutvec, MPI_Count *len,
                             MPI_Datatype *datatype) {
  MPIABI_Count abi_len = *len;
  MPIABI_Datatype abi_datatype = mpi2abi_datatype(*datatype);
  user_fn_c_0(invec, inoutvec, &abi_len, &abi_datatype);
}
...
int MPIABI_Op_create_c(MPIABI_User_function_c *user_fn, int commute,
  ...
  assert(!user_fn_0);
  user_fn_c_0 = user_fn;
  int ierr = MPI_Op_create_c(wrap_user_fn_c_0, commute, &mpi_op);
  *op = mpi2abi_op(mpi_op);
  return mpi2abi_errorcode(ierr);
}
